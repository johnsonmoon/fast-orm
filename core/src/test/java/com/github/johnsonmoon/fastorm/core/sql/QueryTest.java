...
public class QueryTest {
	...
	@Test
	public void testChildQueryCondition() {
		Query query = Query.selectAll().from(Query.select("_id", "price as pc", "create_date as cd").from("car"), "p");
		query.addWhere(Criteria.where("p.cd").between("2018-01-01 00:00:00", "2018-04-20 00:00:00").and("p.pc").is(150000));
		query.limit(5);
		query.offset(0);
		System.out.println(query.getSql());
	}
	...
	public void pagedQueryTest() {
		DatabaseType.CURRENT_DATABASE = DatabaseType.MYSQL;
		Query query = Query.selectAll().from("car").limit(5).offset(0);
		System.out.println(query.getSql());

		DatabaseType.CURRENT_DATABASE = DatabaseType.SQLITE;
		query = Query.selectAll().from("car").limit(5).offset(0);
		System.out.println(query.getSql());

		DatabaseType.CURRENT_DATABASE = DatabaseType.POSTGRESQL;
		query = Query.selectAll().from("car").limit(5).offset(0);
		System.out.println(query.getSql());

		DatabaseType.CURRENT_DATABASE = DatabaseType.INFORMIX;
		query = Query.selectAll().from("car").limit(5).offset(0);
		System.out.println(query.getSql());

		DatabaseType.CURRENT_DATABASE = DatabaseType.ORACLE;
		query = Query.selectAll("c").from("car c").limit(5).offset(0);
		System.out.println(query.getSql());

		DatabaseType.CURRENT_DATABASE = DatabaseType.ORACLE;
		query = Query.select("c.*").from("car c").limit(5).offset(0).addOrderBy(Order.orderBy("id").direction_DESC());
		System.out.println(query.getSql());

		DatabaseType.CURRENT_DATABASE = DatabaseType.DB2;
		query = Query.select("c.*").from("car c").limit(5).offset(0).addOrderBy(Order.orderBy("id").direction_DESC());
		System.out.println(query.getSql());

		DatabaseType.CURRENT_DATABASE = DatabaseType.SQLSERVER;
		query = Query.select("c.*").from("car c").limit(5).offset(0).addOrderBy(Order.orderBy("id").direction_DESC());
		System.out.println(query.getSql());
	}
}
